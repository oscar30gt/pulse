...
TEST(ConcatenatorTest, ConstructionCreatesPorts)
{
    Wire low(4), high(4), out(8);
    Concatenator concatenator(&low, &high, &out);
    EXPECT_TRUE(concatenator.hasInputPort("low"));
    EXPECT_TRUE(concatenator.hasInputPort("high"));
    EXPECT_TRUE(concatenator.hasOutputPort("out"));
    EXPECT_FALSE(concatenator.hasPort("nonexistent"));
}

TEST(ConcatenatorTest, SimpleConcatenation)
{
    Wire low(4), high(4), out(8);
    Concatenator concatenator(&low, &high, &out);
...
TEST(ConcatenatorTest, VaryingWidths)
{
...
TEST(ConcatenatorTest, ExhaustiveTruthTable)
{
...
TEST(ConcatenatorTest, ChainedConcatenators)
{
    Wire a(4), b(4), c(4), mid(8), out(12);
    Concatenator m1(&a, &b, &mid);
    Concatenator m2(&mid, &c, &out);
...
TEST(ConcatenatorTest, ConstructionFailsOnWidthMismatch)
{
